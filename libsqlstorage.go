...
func NewStorage(c LibSQLStorage) (certmagic.Storage, error) {
	var connStr string
	if len(c.URL) > 0 {
		connStr = c.URL
	} else {
	...
	s := &LibSQLStorage{
		db:             database,
		URL:            c.URL,
		LockTTLSeconds: c.LockTTLSeconds,
	}

	caddy.Log().Named("storage.libsql").Debug(fmt.Sprintf("NewStorage %v %v", c, s))
	return s, s.ensureTableSetup()
}

func (s *LibSQLStorage) ensureTableSetup() error {
	...
	defer tx.Rollback()
	caddy.Log().Named("storage.libsql.sql").Debug(fmt.Sprintf("ensureTableSetup"))
	createTable := `
...
expire_at TIMESTAMP
	...
	_, err = tx.ExecContext(ctx, createLockTable)
	if err != nil {
		return err
	}
	return tx.Commit()
...
func (c *LibSQLStorage) CertMagicStorage() (certmagic.Storage, error) {
	return NewStorage(*c)
}

// Provision sets up the LibSQL client and ensures DB schema.
func (c *LibSQLStorage) Provision(ctx caddy.Context) error {
	// Load Environment
	if c.URL == "" {
		c.URL = os.Getenv("LIBSQL_URL")
	}
	if c.LockTTLSeconds == 0 {
		c.LockTTLSeconds = 60
	}

	caddy.Log().Named("storage.libsql").Debug(fmt.Sprintf("Provision %v", c))
	return nil
}
